  ...
  return (
    <div className='flex justify-between px-4 py-3 sm:px-10 border-b bg-white'>
        <div className='flex items-center gap-2 text-xs'>
            <img className='w-36 sm:w-40 max-sm:w-24 cursor-pointer' src={assets.admin_logo} alt="" />
            <p className='px-2.5 py-0.5 border border-gray-500 rounded-full text-gray-600'>
                {adminAtoken ? "Admin" : "Doctor"}
            </p>
        </div>
        <button onClick={logout} className='py-2 px-4 bg-primary text-white text-sm rounded-full cursor-pointer max-sm:text-xs'>Logout</button>
    </div>
  )
}
